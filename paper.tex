...
\begin{figure}[htbp]
  ...
  \framebox{\parbox{0.9\columnwidth}{\centering
    \vspace{4cm}
    \textbf{Magnetometer Time-Series Chart} \\
    \small\textit{Sensor readings over time showing Mx, My, and Mz components.}
    \vspace{4cm}
  }}
  \caption{Magnetometer time-series chart showing sensor readings over time. The chart displays three magnetic field components: \textbf{Mx (pink/light red)} representing the X-axis component, \textbf{My (blue)} representing the Y-axis component, and \textbf{Mz (teal/light green)} representing the Z-axis component, all measured in microteslas (µT). The synchronized view enables identification of anomalies (sudden spikes) and correlation with geographic locations and flight patterns through cross-view brushing and linking. The chart reveals high-frequency, high-variability data with all three components exhibiting significant fluctuations. The Mx and My components show the largest magnitudes and variations, typically oscillating between approximately -1.0\,µT and 1.2\,µT, while the Mz component generally remains closer to zero (ranging between -0.5\,µT and 0.8\,µT) but also displays notable spikes. Distinct behavioral periods are observable: periods of high variability alternating with segments showing pronounced negative excursions in the My component, particularly around low-altitude flight segments. The sudden spikes visible across all components correlate with specific geographic locations and flight conditions, enabling anomaly detection and sensor validation through cross-view coordination.}
